...
const LIST_ITEM_RE = /^([-*â€¢â€“â€”]|(?:\d+|[A-Za-z])[.)])\s+(.+)$/
const BOLD_RE = /\*\*(.+?)\*\*/g
const ARROW_ONLY_RE = /^(?:â†“|â¬‡|->|=>|â†’)+$/
const FLOW_CONNECTOR_RE = /\s*(?:->|=>|â†’|â†“|â¬‡)\s*/
      ...
      {steps.map((step, index) => {
        ...
        return (
          <Fragment key={`step-${index}`}>
            ...
            {index < steps.length - 1 && <div className="section-flow-arrow">â†“</div>}
          </Fragment>
        )
      })}
    </div>
