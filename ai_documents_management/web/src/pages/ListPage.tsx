...
interface FilterOptions {
  don_vi_ban_hanhs: string[]
  loai_van_bans: string[]
  chu_des: string[]
  authors: string[]
}
...
export default function ListPage() {
  ...
  const fetchGuidelines = useCallback(async () => {
    ...
    if (chuDe) params.set('chu_de', chuDe)
    if (donViBanHanh) params.set('don_vi_ban_hanh', donViBanHanh)
    if (loaiVanBan) params.set('loai_van_ban', loaiVanBan)
    if (user?.role === 'admin' && ownerFilter) {
      params.set('owner_user_id', ownerFilter)
    }
  ...
  }, [chuDe, donViBanHanh, loaiVanBan, page, search, ownerFilter, user?.role])
        ...
        <div className="filter-bar" style={{ flexWrap: 'wrap', gap: 12 }}>
          ...
          <div className="form-group" style={{ width: 220, minWidth: 180 }}>
            <select
              ...
              value={donViBanHanh}
              onChange={e => {
                setDonViBanHanh(e.target.value)
                setPage(1)
              }}
            >
              <option value="">Tất cả đơn vị ban hành</option>
              {filterOptions.don_vi_ban_hanhs.map(option => (
          ...
          <div className="form-group" style={{ width: 220, minWidth: 180 }}>
            ...
            >
              ...
              {filterOptions.loai_van_bans.map(option => (
                <option key={option} value={option}>
                  {option}
                </option>
              ))}
          ...
          <div className="form-group" style={{ width: 220, minWidth: 180 }}>
            ...
            >
              ...
              {filterOptions.chu_des.map(option => (
                <option key={option} value={option}>
                  {option}
                </option>
              ))}
              ...
              <tbody>
                {data?.items.map(item => (
                  <tr key={item.guideline_id}>
                    ...
                    <td>{item.chu_de ? <span className="badge badge-default">{item.chu_de}</span> : '-'}</td>
                    <td>
                      {item.active_version ? (
                        <div>
