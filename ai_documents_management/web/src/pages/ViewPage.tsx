...
function findBestSectionForPage(nodes: WorkspaceSectionNode[], page: number): WorkspaceSectionNode | null {
  ...
  // No preceding section: user is before the first real section (e.g. cover page). Don't auto-select a future section â€” that would yank both panes onto a section the user hasn't asked for.
  return null
}
...
function findBestSpatialSectionForLocation(
  ...
  // Before all sections â†’ no auto-jump (consistent with findBestSectionForPage).
  return findBestSectionForPage(nodes, page)
}
